...
task = sky.Task(
    name=f"run-down_channel_sample_das",
    setup="""
echo "Begin setup."                                                           
...
echo "Running down_channel_sample_das on (node_rank, num_node) = ($NODE_RANK, $NUM_NODES)"
python run_downsample_das.py --year $YEAR --node_rank $NODE_RANK --num_nodes $NUM_NODES --jday_start $JDAY_START --jday_end $JDAY_END --minute_end 1 --downsample_folder mbdas_h5_down_channel_sample_list --downsample_type down_channel_sample
""",
    workdir=".",
    num_nodes=1,
...
# # sky.launch(task, cluster_name="down_channel_sample_das")
# sky.exec(task, cluster_name="down_channel_sample_das")
# raise
...
for NODE_RANK in range(NUM_NODES):
    ...
    cluster_name = f"down_channel_sample_das-{NODE_RANK:03d}"
    ...
    print(f"Running down_channel_sample_das on (rank={NODE_RANK}, num_node={NUM_NODES}) of {cluster_name}")
