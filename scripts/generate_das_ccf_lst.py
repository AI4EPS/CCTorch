...
'''Generate files list from the paths on GCloud, based on the files after doing down_channel_sample. 
   The generated files list will be used for cctorch the DAS data.'''
...
def get_args_parser(add_help=True):
    ...
    parser = argparse.ArgumentParser(description="Generate cctorch_das list", add_help=add_help)
    parser.add_argument(
        "--files_lst",
    ...
    parser.add_argument("--save_path", default="das_preprocess/data_list_3", type=str, help="save path for downsampled list")
...
def gen_das_ccf_lst(year=2024, jday_start=1, jday_end=366, config=None):
    protocol = config["protocol"]
    bucket = config["bucket"]
    load_path = config["load_path"]
    save_path = config["save_path"]
    channel_lst = config["channel_lst"]
    ...
    for jday in jday_lst:
        ...
        try:
            down_channel_sample_file_lst = fs.ls(f'{bucket}/{load_path}/{year}/{jday}/down_channel_sample')
            for file in down_channel_sample_file_lst:
                hr_min = file.split("/")[-1].split("-")[-3].split("T")[-1][:4]
                txt_save_lst = ['file_name,channel_index\n']
                for channel_index in channel_lst:
                    txt_save_lst += [f"{file},{channel_index}\n"]

                file_save = f'data_list_mb_{date_str}T{hr_min}.txt'

                with open(file_save, 'w') as f:
                    for line in txt_save_lst:
                        f.write(line)
...
if __name__ == "__main__":
    ...
    config = {"protocol": args.protocol,
              ...
              "channel_lst": list(range(30))}

    gen_das_ccf_lst(year=year, jday_start=jday_start, jday_end=jday_end, config=config)